...
def plot_filtered_sales_by_daily(data, Customer):
    filtered_data = data[data['고객'].isin(Customer)]    
...
def plot_filtered_sales_by_month(data, Customer):
    ...
    filtered_data = df[df['고객'].isin(Customer)]    
...
def plot_filtered_sales_by_item(data, items):    
    filtered_data = data[data['제품'].isin(items)]
...
def plot_filtered_sales_by_customer(data, Customer):
    filtered_data = data[data['고객'].isin(Customer)]
    sale_counts = filtered_data.groupby('고객')['판매량'].sum()
...
def plot_filtered_sales_heatmap(data, customers):
    ...
    filtered_data = data[data['고객'].isin(customers)]
...
def plot_filtered_production_by_item(data, items):    
    filtered_data = data[data['제품'].isin(items)]
    sale_counts = filtered_data.groupby('제품')['생산량'].sum()
...
def plot_filtered_psi_by_month(data, items):
    ...
    prod_counts = filtered_data.groupby('YearMonth')['생산량'].sum()
    sale_counts = filtered_data.groupby('YearMonth')['판매량'].sum()
...
def plot_filtered_psi_by_daily(data, item):
    ...
    prod_counts = filtered_data.groupby('Date')['생산량'].sum()
    sale_counts = filtered_data.groupby('Date')['판매량'].sum()
    inv_counts = filtered_data.groupby('Date')['재고량'].sum()
    ...
    # 5. 그래프 꾸미기
    ax.set_title('일별 상산량, 판매량 및 재고량 비교', fontsize=15)
