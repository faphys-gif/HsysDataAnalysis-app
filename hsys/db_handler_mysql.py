...
SQL_SD02_DATA = \
	...
	"   ml03.total_qty AS 'Total Sales Quantity', ml03.total_cnt AS 'Total Sales Count',  " \
    "   ml03.avg_qty AS 'Average Sales Quantity', ml03.std_qty AS 'Standard Sales Deviation',  " \
    "   ml03.cv AS 'CV', ml03.adi as 'ADI'  " \
    " from ML03  " \
    "inner join MM01 on MM01.mat_id = ML03.mat_id " \
    "where ML03.biz_id = 31 and ML03.ml_id = 8; " \
...
def load_dataset_ml04_stat(biz_id):
    SQL_STATMENT = \
        "select ML04.trx_id, ML04.biz_id, ML04.cluster_code AS 'Cluster Code', ML04.cluster_name AS 'Cluster Name', " \
        "       ML04.mat_cnt AS 'Num. Of Items', round(ML04.ship_qty/ML04.mat_cnt) AS 'Avg. Sales Qty', " \
        "       round(ML04.ship_cnt/mat_cnt) AS 'Avg. Sales Count', round(ML04.cv,4) AS 'CV',  round(ML04.adi,4) AS 'ADI', " \
        "       ML04.svc_level AS 'Service Level', date_format(ML04.crt_dt,'%Y-%m-%d %r') AS '생성일자' " \
        "from ML04 " \
        "where ML04.biz_id = 31 and ml_id = 8;  " \
